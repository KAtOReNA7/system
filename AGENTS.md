...
- 当前需要用户手动填写的私有补表为 `data/private-output/m2-readiness/M2-classification-aux-tag-fill-pack-cn-v3.xlsx`：`01_分类需人工确认` 有 257 行，`02_辅助标签需核对` 有 51 行。该文件禁止提交。
...
- 作者、版权开始、版权到期、作品状态、音频版权状态已在本地文件级 staging 候选中闭环；在分类、辅助标签、特殊属性标签 formal 主数据缺口闭环前，禁止进入 M3 formal execution。
...
- 禁止绕过分类、辅助标签、特殊属性标签缺口；这些字段必须与作者、版权日期、作品状态、音频版权状态分开单独治理。
