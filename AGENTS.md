...
- LG01 头部保护尾段修正模型 v0.2（`M2-WORK-HPSR02`）已在独立 outcome 打开前
  ...
  WORK_CHANNEL 继续 `PARTIAL_NOT_ACTIVE`。独立 outcome 已由此前结果前工程尝试打开，
  但完整结果仍为 0；本轮用户已授权在参数完整性门禁通过后完成唯一一次首个独立评价。
  prospective final holdout 必须保持未打开，active candidate、自动化批准和 production
  权限继续为空。
