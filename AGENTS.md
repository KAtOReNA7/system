...
- `calibration-spec-c2r-v1.1-amendment`、正式现金内核、as-of 审计、三套 actual、桥接和自动测试已建立；C2-R.1 尚未重新训练或调参，C2/C3 未开始。final holdout、embargo shadow、deferred 60-month labels 仍 sealed；状态保持 `not_for_formal_decision`，未 release，未进入 M3。
...
任何 M3 设计、实现或评估前，必须先提醒用户：M2 隔离本地正式执行已完成，旧 v1.1 conditional 已被用户拒绝；calibration-spec-v1.2 comparator 修正、B0b-B4 replay 和 Gate A 已完成，C1 development 与 legacy-target C2-R development 均已执行但明确 FAIL。legacy C2-R 不具备 formal-cash 指标资格；C2-R.1 未训练，C2/C3、业务抽检、最终 holdout、验收和 release 均未开始或完成。
...
- M2 隔离本地正式执行与严格对账已完成；旧 export 仅为 `prepared` 且已被用户拒绝。calibration-spec-v1.2、faithful B0b/B4 身份修正、严格 comparator、B0b-B4 replay 和 Gate A 均已完成；C1 development 与 legacy-target C2-R development 已执行并均为 FAIL。legacy C2-R 只作历史审计；C2-R.1 尚未训练，C2/C3 未开始。通过后续另行授权的 formal-cash 候选开发、业务抽检、final holdout 与明确批准前禁止进入 M3 formal execution。
