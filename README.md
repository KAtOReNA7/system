...
| 最新受控研究结果是什么？ | LG01 头部保护分段路由模型 v0.1（`M2-WORK-HPSR01`）的原机械合同不支持结论保持冻结，科学解释仍为单起点证据不足（`M2_HPSR01_CONTRACT_UNSUPPORTED_SCIENTIFICALLY_INCONCLUSIVE`）。LG01 头部保护尾段修正模型 v0.2（`M2-WORK-HPSR02`）的独立结果入口已经打开，但尚未形成完整结果；不可变冻结参数权威已决定，当前等待私有完整性门禁（`M2_HPSR02_FROZEN_PARAMETER_AUTHORITY_DECIDED_PENDING_PRIVATE_INTEGRITY_GATE`） |
...
| M2 LG01 头部保护尾段修正 v0.2 | 不可变冻结参数权威已决定，等待私有完整性门禁（`M2_HPSR02_FROZEN_PARAMETER_AUTHORITY_DECIDED_PENDING_PRIVATE_INTEGRITY_GATE`） | 头部 50% 和中部 30% 现金带精确使用冻结 LG01，仅尾部 20% 现金带使用既有冻结修正；当前账单已形成 `2026-03` 起点输入和 `2026-04` 至 `2026-06` 的作品总额 actual，但尚无完整独立成绩，prospective final holdout 未打开 |
...
| 头部保护尾段修正研究 | LG01 头部保护尾段修正模型 v0.2（`M2-WORK-HPSR02`）的独立评价已获准恢复，等待不可变冻结参数完整性门禁 | 头部 50% 和中部 30% 现金带逐行精确使用 LG01，仅尾部 20% 现金带应用冻结修正；旧 577 行只作参数谱系快照（`PARAMETER_LINEAGE_SNAPSHOT`），当前账单不得重估参数；不是活动候选或运行管线 |
...
| 头部保护尾段修正 | LG01 头部保护尾段修正模型 v0.2（LG01 Head-Protected Tail-Band Correction Model v0.2，`M2-WORK-HPSR02`） | 已决定不可变冻结参数权威，等待私有完整性门禁且未激活（`candidate_pending_immutable_parameter_integrity_gate_not_active`） | 独立结果入口已打开但尚无完整成绩；当前账单只形成起点输入与作品总额 actual，不得派生冻结参数；未打开 prospective final holdout、无自动化批准 |
...
冻结；HPSR02 独立评价实验
（`M2-EXP-LG01-HEAD-PROTECTED-TAIL-BAND-CORRECTION-02`）已打开独立结果入口并
保留两次历史阻断检查点与三次结果前工程停止；当前等待不可变冻结参数完整性门禁，
尚无完整独立结果。prospective final holdout 未打开，活动候选和自动化批准仍为空。
...
当前独立结果入口已打开但尚无完整预测或成绩，等待私有参数完整性门禁；事后尾部
20% 聚合算术明确不是模型证据。
...
- [HPSR02 独立评价当前检查点](docs/analysis/m2-current/M2-head-protected-tail-band-correction-independent-evaluation-v0.2.md)
...
| 主题 | 当前入口 |
|---|---|
| 最新状态 | [M2 当前状态索引 v0.53](docs/analysis/m2-v2/M2-v2-current-state-index-v0.53.md) |
