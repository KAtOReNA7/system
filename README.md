...
| 后续渠道金额设计到哪一步？ | 出版行业渠道起点可见现金锚金额模型 v0.1（`M2-CHAN-PSC02`）保持历史源权威阻断且无性能证据；出版行业渠道直接现金尺度条件金额模型 v0.1（Publishing-Scale Channel Direct-Cash Conditional Amount Model v0.1，`M2-CHAN-PSC03`；原始候选 `M2-CHAN-PSC03-RAW`）已完成唯一开发重放：primary WAPE 54.2647%，strict WAPE 297.0822%，最终不支持继续开发（`PSC03_DEVELOPMENT_NOT_SUPPORTED`） |
...
| M2 出版行业规模适配 | PSC01 原始候选冻结失败；PSC02 阻断证据已集成；[PSC03 唯一开发重放](docs/analysis/m2-current/M2-publishing-scale-channel-direct-cash-development-evaluation-v0.1.md)已完成并冻结（`PSC03_DEVELOPMENT_NOT_SUPPORTED`） | PSC03 主口径改善 PSC01，但 strict 短周期严重放大且全面落后 LG01；尺度假设和候选优越性均未通过，不授权重跑或独立评价 |
...
最新状态以 [M2 当前状态索引 v0.60](docs/analysis/m2-v2/M2-v2-current-state-index-v0.60.md)
...
| 渠道预测研究 | 出版行业规模适配渠道核心（`M2-CHAN-PSC01`）已执行失败；直接现金尺度后继模型（`M2-CHAN-PSC03`）唯一开发重放亦不支持继续开发 | PSC03 算术与准 Gamma 臂仅为诊断；准 Poisson 唯一 raw candidate 只形成一次。primary WAPE 54.2647%，strict WAPE 297.0822%，`activeCandidate=null` |
...
| 渠道预测 | 出版行业渠道直接现金尺度条件金额模型 v0.1（Publishing-Scale Channel Direct-Cash Conditional Amount Model v0.1，`M2-CHAN-PSC03`） | 已执行失败开发候选（`failed_development_candidate`；`PSC03_DEVELOPMENT_NOT_SUPPORTED`） | 唯一开发重放和首次完整 raw 已冻结；不重跑、不打开独立评价，`activeCandidate=null` |
...
- [M2 当前状态索引 v0.60](docs/analysis/m2-v2/M2-v2-current-state-index-v0.60.md)
...
周期的 2,000 次作品级 bootstrap 95% 区间为 [-14.4218%, 1.8034%]，跨 0；尺度假设与
候选优越性门禁均未通过。最终状态为不支持继续开发（`PSC03_DEVELOPMENT_NOT_SUPPORTED`），
不授权重跑、独立评价、later-origin、final holdout、taxonomy、production 或 automation。
...
3. 再读 [M2 模型目录与成绩总账](docs/analysis/m2-current/M2-model-catalog-and-scorecard-v1.md)，
   理解模型、实验、状态码和当前角色；
4. 最后读 [M2 当前状态索引 v0.60](docs/analysis/m2-v2/M2-v2-current-state-index-v0.60.md)，
   查看最新结论、阻断项和下一步。
...
| 主题 | 当前入口 |
|---|---|
| 最新状态 | [M2 当前状态索引 v0.60](docs/analysis/m2-v2/M2-v2-current-state-index-v0.60.md) |
