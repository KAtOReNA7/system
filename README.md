...
legacy-target C2-R development 已完成但结论为 `FAIL`，其结果继续作为历史目标口径证据，不是 formal-cash 指标，也不得与 C2-R.1 直接比较；旧 development 写入口现已 fail-closed，只保留历史验证入口。C2-R.1 尚未在新目标上重新训练或调参，C2/C3 未开始；所有结果仍为 `not_for_formal_decision`，final holdout、embargo shadow 和 deferred 60-month labels 仍 sealed，未 release，未进入 M3。
