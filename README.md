...
| 后续渠道金额设计到哪一步？ | 出版行业渠道起点可见现金锚金额模型 v0.1（`M2-CHAN-PSC02`）保持历史源权威阻断且无性能证据；新的出版行业渠道直接现金尺度条件金额模型 v0.1（Publishing-Scale Channel Direct-Cash Conditional Amount Model v0.1，`M2-CHAN-PSC03`；原始候选 `M2-CHAN-PSC03-RAW`）已完成结果前预注册和公共 synthetic 完整路径，私有开发重放尚未执行（`NO_MODEL_PERFORMANCE_EVIDENCE`） |
...
| M2 出版行业规模适配 | 首个 PSC01 原始候选已冻结失败（`M2_PUBLISHING_SCALE_CORE_FAIL`）；PSC02 阻断证据已集成；[PSC03 结果前预注册](docs/analysis/m2-current/M2-publishing-scale-channel-direct-cash-quasi-poisson-preregistration-v0.1.md)和公共实现已完成 | PSC03 只复用冻结 PSC01 权威，不要求 PSC02 四字段；当前等待全部公共门禁与 pre-execution exact-head 双平台 CI，没有私有候选成绩（`NO_MODEL_PERFORMANCE_EVIDENCE`） |
...
最新状态以 [M2 当前状态索引 v0.59](docs/analysis/m2-v2/M2-v2-current-state-index-v0.59.md)
...
| 渠道预测研究 | 出版行业规模适配渠道核心（`M2-CHAN-PSC01`）已执行失败；直接现金尺度后继模型（`M2-CHAN-PSC03`）已预注册但尚未私有执行 | PSC01 raw candidate 保持冻结；PSC03 的算术与准 Gamma 臂仅为诊断，准 Poisson 臂才是唯一原始候选，公共 synthetic 通过不等于模型成绩 |
...
| 渠道预测 | 出版行业渠道直接现金尺度条件金额模型 v0.1（Publishing-Scale Channel Direct-Cash Conditional Amount Model v0.1，`M2-CHAN-PSC03`） | 未激活的结果前开发候选（`inactive_preregistered_development_candidate_not_executed`） | 公共实现和 synthetic 完整路径已验证；唯一私有开发重放尚未执行，`activeCandidate=null` |
...
- [M2 当前状态索引 v0.59](docs/analysis/m2-v2/M2-v2-current-state-index-v0.59.md)
...
本轮另行预注册出版行业渠道直接现金尺度条件金额模型 v0.1
（`M2-CHAN-PSC03`；原始候选 `M2-CHAN-PSC03-RAW`；实验
`M2-EXP-PUBLISHING-SCALE-CHANNEL-DIRECT-CASH-03`）。它只复用冻结 PSC01 的人口、训练
信息、18 个特征、basis、occurrence、actual、origin、horizon 与渠道结构；不读取 PSC02
的四个缺失字段，也不把 `extra=3` 作为门禁。算术层级与直接准 Gamma 层级是诊断，直接
准 Poisson 层级是唯一原始候选。当前公共完整路径已通过
（`PSC03_PUBLIC_SYNTHETIC_FULL_PATH_PASSED`），但私有开发重放尚未执行，因此没有 WAPE、
FVA、bootstrap 或科学状态（`NO_MODEL_PERFORMANCE_EVIDENCE`）。
...
3. 再读 [M2 模型目录与成绩总账](docs/analysis/m2-current/M2-model-catalog-and-scorecard-v1.md)，
   理解模型、实验、状态码和当前角色；
4. 最后读 [M2 当前状态索引 v0.59](docs/analysis/m2-v2/M2-v2-current-state-index-v0.59.md)，
   查看最新结论、阻断项和下一步。
...
| 主题 | 当前入口 |
|---|---|
| 最新状态 | [M2 当前状态索引 v0.59](docs/analysis/m2-v2/M2-v2-current-state-index-v0.59.md) |
...
| 出版规模渠道实验 | [开发评价](docs/analysis/m2-current/M2-current-publishing-scale-channel-development-v0.1.md) · [可预测性诊断](docs/analysis/m2-current/M2-current-publishing-scale-channel-forecastability-v0.1.md) · [条件金额尺度根因审计](docs/analysis/m2-current/M2-publishing-scale-channel-amount-scale-root-cause-audit-v0.1.md) · [PSC02 起点可见现金锚金额预注册](docs/analysis/m2-current/M2-publishing-scale-channel-origin-visible-cash-anchor-preregistration-v0.1.md) · [PSC02 当前执行与源恢复审计](docs/analysis/m2-current/M2-psc02-pr40-execution-completeness-and-source-authority-recovery-audit-v0.1.md) · [PSC03 直接现金尺度结果前预注册](docs/analysis/m2-current/M2-publishing-scale-channel-direct-cash-quasi-poisson-preregistration-v0.1.md) · [PSC03 开发评价状态](docs/analysis/m2-current/M2-publishing-scale-channel-direct-cash-development-evaluation-v0.1.md) |
