...
| 后续渠道金额设计到哪一步？ | 出版行业渠道起点可见现金锚金额模型 v0.1（Publishing-Scale Channel Origin-Visible Cash-Anchor Amount Model v0.1，`M2-CHAN-PSC02`；raw variant `M2-CHAN-PSC02-RAW`）已实现并通过公共 synthetic 合同；尚未拟合、预测或评价，正等待执行前 exact-head 双平台 CI（`M2_PSC02_IMPLEMENTED_AWAITING_EXACT_HEAD_CI_AND_CONTROLLED_DEVELOPMENT_REPLAY`） |
...
| M2 出版行业规模适配 | 首个 PSC01 原始候选已冻结失败（`M2_PUBLISHING_SCALE_CORE_FAIL`）；[条件金额尺度根因审计](docs/analysis/m2-current/M2-publishing-scale-channel-amount-scale-root-cause-audit-v0.1.md)已完成；[PSC02 实现就绪](docs/analysis/m2-current/M2-publishing-scale-channel-origin-visible-cash-anchor-implementation-readiness-v0.1.md)已登记 | PSC02 冻结 PSC01 occurrence，只改变条件正金额；核心已实现但尚无真实结果，当前 private 元数据预检因缺少 component/revision/time 权威字段而失败关闭 |
...
最新状态以 [M2 当前状态索引 v0.56](docs/analysis/m2-v2/M2-v2-current-state-index-v0.56.md)
...
| 渠道预测研究 | 出版行业规模适配渠道核心（Publishing-Scale Channel Core，`M2-CHAN-PSC01`）已执行失败 | raw candidate 已冻结；只读根因审计确认估计器尺度收缩且实现正确，后验统一 scalar 不能修复渠道构成；不授权同窗调参或下一修订实现 |
...
- [M2 当前状态索引 v0.56](docs/analysis/m2-v2/M2-v2-current-state-index-v0.56.md)
...
失败关闭执行入口已实现，但尚未拟合、预测或评价。执行前 private 元数据预检只确认当前
源权威缺少 `componentId`、`revisionId`、`effectiveAt` 和 `availableAt`，没有打开候选结果。
...
3. 再读 [M2 模型目录与成绩总账](docs/analysis/m2-current/M2-model-catalog-and-scorecard-v1.md)，
   理解模型、实验、状态码和当前角色；
4. 最后读 [M2 当前状态索引 v0.56](docs/analysis/m2-v2/M2-v2-current-state-index-v0.56.md)，
   查看最新结论、阻断项和下一步。
...
| 主题 | 当前入口 |
|---|---|
| 最新状态 | [M2 当前状态索引 v0.56](docs/analysis/m2-v2/M2-v2-current-state-index-v0.56.md) |
...
| 出版规模渠道实验 | [开发评价](docs/analysis/m2-current/M2-current-publishing-scale-channel-development-v0.1.md) · [可预测性诊断](docs/analysis/m2-current/M2-current-publishing-scale-channel-forecastability-v0.1.md) · [条件金额尺度根因审计](docs/analysis/m2-current/M2-publishing-scale-channel-amount-scale-root-cause-audit-v0.1.md) · [起点可见现金锚金额预注册](docs/analysis/m2-current/M2-publishing-scale-channel-origin-visible-cash-anchor-preregistration-v0.1.md) · [PSC02 实现就绪](docs/analysis/m2-current/M2-publishing-scale-channel-origin-visible-cash-anchor-implementation-readiness-v0.1.md) |
