...
const state = {
  ...
  m3SelectedTopicId: "SYN-TOPIC-0001",
  m3TopicFilters: {
    query: "",
    source: "",
    readiness: "",
...
async function getM3Json(path) {
  const response = await fetch(path, { headers: { accept: "application/json" } });
  const body = await response.json();
  if (!response.ok) {
    const error = new Error(body?.error?.message || "M3 request failed");
    error.statusCode = response.status;
    error.payload = body;
    throw error;
  }
...
function renderM3DatasetPanel(dataset) {
  return `
    <section class="m2-safety-panel" data-testid="m3-safety-panel">
      <div class="meta-row">
        <span class="badge warn">fixture-only</span>
        <span class="badge ok">synthetic marker</span>
        <span class="badge warn">formal M3 blocked</span>
      </div>
      <div class="cards three compact-cards">
        <article class="card">
          <h3>数据集</h3>
          ${renderMetric("dataset.mode", dataset?.mode || "fixture")}
          ${renderMetric("source", dataset?.source || "m3-new-product-static-synthetic-fixture")}
          ${renderMetric("formalEvaluationAllowed", dataset?.formalEvaluationAllowed ?? false)}
        </article>
        <article class="card">
          <h3>M3 边界</h3>
          ${renderMetric("m3FormalExecutionAllowed", dataset?.m3FormalExecutionAllowed ?? false)}
          ${renderMetric("dependsOnM2FormalReadiness", dataset?.dependsOnM2FormalReadiness ?? true)}
          <p class="pagination-note">当前只读页面仅用于新品评估 fixture/prototype。</p>
        </article>
        <article class="card">
          <h3>正式状态</h3>
          <p class="blocked-copy">M3 formal execution remains blocked until M2 formal readiness and explicit authorization are complete.</p>
        </article>
      </div>
    </section>
  `;
...
function renderM3Error(error, options = {}) {
  const payload = error.payload || {};
  const apiError = payload.error || {};
  const code = apiError.code || "request_failed";
  const message = ERROR_MESSAGES[code] || "新品评估信息暂不可读取。";
  return `
    <div class="error-state ${code === "formal_data_blocked" ? "blocked-state" : ""}">
      <h3>${escapeHtml(options.title || "新品评估状态暂不可用")}</h3>
      <p>${escapeHtml(message)}</p>
      <p class="technical-code">技术码：<code>${escapeHtml(code)}</code></p>
      ${apiError.requestId ? `<p class="pagination-note">requestId: ${escapeHtml(apiError.requestId)}</p>` : ""}
    </div>
...
function setM3ErrorPageState(page, error) {
  const code = error.payload?.error?.code;
  if (code === "formal_data_blocked") {
    setPageState(page, "blocked");
    return;
  }
  setPageState(page, error.statusCode === 404 ? "not found" : "error");
}
...
async function renderM3Overview() {
  setPageState("m3-overview", "loading");
  document.querySelector("#m3OverviewContent").innerHTML = '<div class="loading">加载新品评估总览…</div>';
  try {
    const data = await getM3Json("/api/m3/new-products/topics/overview");
    setPageState("m3-overview", "success");
    document.querySelector("#m3OverviewContent").innerHTML = `
      ${renderM3DatasetPanel(data.dataset)}
      <div class="cards three">
        <article class="card">
          <h3>选题规模</h3>
          ${renderMetric("total topics", data.summary.totalTopics)}
          ${renderMetric("ready topics", data.summary.readyTopics)}
          ${renderMetric("blocked topics", data.summary.blockedTopics)}
          ${renderMetric("draft topics", data.summary.draftTopics)}
        </article>
        <article class="card">
          <h3>PRD 边界</h3>
          ${renderMetric("final comparator cap", data.summary.finalComparatorCap)}
          ${renderMetric("backtest checkpoints", data.summary.backtestCheckpoints.join(", "))}
          ${renderMetric("linked topics", data.summary.linkedTopics)}
        </article>
        <article class="card">
          <h3>正式门禁</h3>
          <p class="blocked-copy">M3 只读 fixture 已就绪；formal execution 仍需 M2 readiness rerun 和用户单独授权。</p>
        </article>
      </div>
      <div class="cards three">
        ${renderDistribution("source distribution", data.distribution.source)}
        ${renderDistribution("readiness distribution", data.distribution.readiness)}
    ...
    setM3ErrorPageState("m3-overview", error);
    document.querySelector("#m3OverviewContent").innerHTML = renderM3Error(error);
  }
}

async function renderM3Topics() {
  setPageState("m3-topics", "loading");
  document.querySelector("#m3TopicsContent").innerHTML = '<div class="loading">加载新品选题库…</div>';
  const query = buildQuery(state.m3TopicFilters);
  try {
    const data = await getM3Json(`/api/m3/new-products/topics?${query}`);
    if (!data.items.length) {
      setPageState("m3-topics", "empty");
      document.querySelector("#m3TopicsContent").innerHTML = `
        ${renderM3DatasetPanel(data.dataset)}
        ${renderM3TopicFilters()}
        ${renderEmpty("暂无符合条件的新品选题", "当前筛选条件没有匹配记录。")}
      `;
      attachM3TopicFilterHandlers();
      return;
    }
    setPageState("m3-topics", "success");
    document.querySelector("#m3TopicsContent").innerHTML = `
      ${renderM3DatasetPanel(data.dataset)}
      ${renderM3TopicFilters()}
      <div class="table-wrap">
        ...
        <table>
          ...
          <tbody>
            ${data.items.map((item) => `
              <tr>
                <td><a href="#m3-detail:${encodeURIComponent(item.topicId)}">${escapeHtml(item.topicId)}</a></td>
                ...
                <td>${renderM2ReadinessBadge(item.readiness)}</td>
                <td>${renderM2RatingBadge(item.rating)}</td>
                <td>${escapeHtml(item.fiveYearBase)}</td>
                <td>${escapeHtml(item.firstYearForecast)}</td>
                <td>${escapeHtml(item.finalComparatorCount)}</td>
                <td>${escapeHtml(item.topicWorkLinkStatus)}</td>
                <td><a href="#m3-detail:${encodeURIComponent(item.topicId)}">View detail</a></td>
              </tr>
            `).join("")}
          </tbody>
        </table>
    ...
    setM3ErrorPageState("m3-topics", error);
    document.querySelector("#m3TopicsContent").innerHTML = renderM3Error(error);
  }
}

function renderM3TopicFilters() {
  const filters = state.m3TopicFilters;
  return `
    <form class="filter-panel" id="m3TopicFilters">
      <label>Search
        <input name="query" value="${escapeAttribute(filters.query)}" placeholder="SYN-TOPIC">
      </label>
      <label>Source
        <select name="source">
          ${option("", "全部", filters.source)}
          ${["publication", "web_original"].map((value) => option(value, value, filters.source)).join("")}
        </select>
      </label>
      <label>Readiness
        <select name="readiness">
          ${option("", "全部", filters.readiness)}
          ${["ready", "blocked", "draft"].map((value) => option(value, value, filters.readiness)).join("")}
        </select>
      </label>
      <label>Status
        <select name="status">
          ${option("", "全部", filters.status)}
          ${["evaluation_ready", "input_confirmed", "readiness_blocked", "material_parse_pending", "linked_after_sales"].map((value) => option(value, value, filters.status)).join("")}
        </select>
      </label>
      <label>Rating
        <select name="rating">
          ${option("", "全部", filters.rating)}
          ${["S+", "S", "A", "B", "C", "D", "E", "blocked"].map((value) => option(value, value, filters.rating)).join("")}
        </select>
      </label>
      <label>Sort
        <select name="sort">
          ${["createdAt.desc", "fiveYearBase.desc", "firstYearForecast.desc", "rating.asc"].map((value) => option(value, value, filters.sort)).join("")}
        </select>
      </label>
      <button type="submit" class="secondary">查询</button>
      <button type="button" class="secondary" data-m3-reset-topics>Reset filters</button>
    </form>
  `;
}

function attachM3TopicFilterHandlers() {
  const form = document.querySelector("#m3TopicFilters");
  if (!form) {
    return;
  }
  form.addEventListener("submit", (event) => {
    event.preventDefault();
    const formData = new FormData(form);
    state.m3TopicFilters = {
      query: String(formData.get("query") || ""),
      source: String(formData.get("source") || ""),
      readiness: String(formData.get("readiness") || ""),
      status: String(formData.get("status") || ""),
      rating: String(formData.get("rating") || ""),
      sort: String(formData.get("sort") || "createdAt.desc"),
      page: 1,
      pageSize: 20
    };
    renderM3Topics();
  });
  form.querySelector("[data-m3-reset-topics]")?.addEventListener("click", () => {
    state.m3TopicFilters = {
      query: "",
      source: "",
      readiness: "",
...
async function renderM3Detail() {
  setPageState("m3-detail", "loading");
  document.querySelector("#m3DetailContent").innerHTML = '<div class="loading">加载新品评估详情…</div>';
  try {
    const data = await getM3Json(`/api/m3/new-products/topics/${encodeURIComponent(state.m3SelectedTopicId)}`);
    setPageState("m3-detail", "success");
    document.querySelector("#m3DetailContent").innerHTML = `
      ${renderM3DatasetPanel(data.dataset)}
      <div class="context-note">
        <a href="#m3-topics">Back to topic library</a>
        <p>Fixture topic input is structured metadata only; raw material content is not stored in this public prototype.</p>
      </div>
      <div class="cards three">
        <article class="card">
          <h3>核心结论</h3>
          ${renderMetric("topicId", data.topic.topicId)}
          ${renderMetric("title", data.topic.title)}
          ${renderMetric("rating", data.rating.value)}
          ${renderMetric("fiveYearBase", data.forecast.fiveYearBase)}
          ${renderMetric("firstYearForecast", data.forecast.firstYearForecast)}
        </article>
        <article class="card">
          <h3>输入 readiness</h3>
          ${renderMetric("readiness", data.inputSnapshot.readiness)}
          ${renderMetric("gap count", data.inputSnapshot.gaps.length)}
          ${renderMetric("publicationOrWebModel", data.inputSnapshot.publicationOrWebModel)}
          ${renderMetric("confirmedByOperator", data.inputSnapshot.confirmedByOperator)}
        </article>
        <article class="card">
          <h3>材料与内容</h3>
          ${renderMetric("fileMetadataOnly", data.material.fileMetadataOnly)}
          ${renderMetric("rawMaterialStored", data.material.rawMaterialStored)}
          ${renderMetric("materialTypes", data.material.materialTypes.join(", "))}
          ${renderMetric("estimatedAudioHours", data.contentFit.estimatedAudioHours)}
        </article>
      </div>
      <div class="cards three">
        <article class="card">
          <h3>对标</h3>
          <ul class="explain-list">
            ${data.comparators.map((item) => `<li>${escapeHtml(item.comparatorWorkId)} · ${escapeHtml(item.reason)} · final=${escapeHtml(item.selectedAsFinal)}</li>`).join("")}
          </ul>
        </article>
        <article class="card">
          <h3>作者排位</h3>
          ${renderMetric("enabled", data.authorRanking.enabled)}
          ${renderMetric("comparableWorks", data.authorRanking.comparableWorks)}
          ${renderMetric("rankBand", data.authorRanking.rankBand)}
          <p class="pagination-note">${escapeHtml(data.authorRanking.rationale)}</p>
        </article>
        <article class="card">
          <h3>预测结构</h3>
          ${renderMetric("outputType", data.forecast.outputType)}
          ${renderMetric("range.base", data.forecast.range?.base)}
          ${renderMetric("range.pessimistic", data.forecast.range?.pessimistic)}
          ${renderMetric("range.optimistic", data.forecast.range?.optimistic)}
        </article>
      </div>
      <div class="cards three">
        <article class="card">
          <h3>评级与风险</h3>
          ${renderMetric("basis", data.rating.basis)}
          ${renderMetric("noDevelopDecisionOutput", data.rating.noDevelopDecisionOutput)}
          ${renderMetric("noResourceInvestmentLevel", data.rating.noResourceInvestmentLevel)}
          <ul class="explain-list">
            ${data.risks.map((risk) => `<li>${escapeHtml(risk.riskCode)} · ${escapeHtml(risk.severity)}</li>`).join("")}
          </ul>
        </article>
        <article class="card">
          <h3>选题与老品关联</h3>
          ${renderMetric("status", data.topicWorkLink.status)}
          ${renderMetric("standardWorkId", data.topicWorkLink.standardWorkId)}
          ${renderMetric("oneTopicOneWork", data.topicWorkLink.oneTopicOneWork)}
          ${renderMetric("oneWorkOneFormalTopic", data.topicWorkLink.oneWorkOneFormalTopic)}
        </article>
        <article class="card">
          <h3>回测计划</h3>
          ${renderMetric("baseline", data.backtestPlan.baseline)}
          ${renderMetric("checkpoints", data.backtestPlan.checkpoints.join(", "))}
          ${renderMetric("currentOutcome", data.backtestPlan.currentOutcome)}
        </article>
      </div>
    `;
  } catch (error) {
    setM3ErrorPageState("m3-detail", error);
    document.querySelector("#m3DetailContent").innerHTML = renderM3Error(error, {
      title: error.statusCode === 404 ? "新品评估详情未找到" : undefined
    });
  }
}

async function renderM3Gaps() {
  setPageState("m3-gaps", "loading");
  document.querySelector("#m3GapsContent").innerHTML = '<div class="loading">加载新品数据缺口…</div>';
  const query = buildQuery(state.m3GapFilters);
  try {
    const data = await getM3Json(`/api/m3/new-products/readiness-gaps?${query}`);
    setPageState(data.items.length ? "m3-gaps" : "m3-gaps", data.items.length ? "success" : "empty");
    document.querySelector("#m3GapsContent").innerHTML = `
      ${renderM3DatasetPanel(data.dataset)}
      <div class="context-note">
        <strong>Input readiness</strong>
        <p>这些缺口只来自 synthetic fixture，用于验证 M3 必填字段和 formal gate。</p>
      </div>
      <div class="table-wrap">
        ${renderTableHint()}
        <table>
          ...
          <tbody>
            ${data.items.map((item) => `
              <tr>
                ...
                <td>${renderM2ReadinessBadge(item.readiness)}</td>
                <td>${escapeHtml(item.blocksFormalEvaluation)}</td>
                <td>${escapeHtml(item.suggestedOwnerAction)}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
    ...
    setM3ErrorPageState("m3-gaps", error);
    document.querySelector("#m3GapsContent").innerHTML = renderM3Error(error);
  }
}

async function renderM3Backtests() {
  setPageState("m3-backtests", "loading");
  document.querySelector("#m3BacktestsContent").innerHTML = '<div class="loading">加载新品回测…</div>';
  try {
    const backtests = await getM3Json("/api/m3/new-products/backtests");
    const algorithms = await getM3Json("/api/m3/new-products/algorithm-versions");
    const detailId = backtests.items[0]?.id;
    const detail = detailId ? await getM3Json(`/api/m3/new-products/backtests/${encodeURIComponent(detailId)}`) : null;
    setPageState("m3-backtests", backtests.items.length ? "success" : "empty");
    document.querySelector("#m3BacktestsContent").innerHTML = `
      ${renderM3DatasetPanel(backtests.dataset)}
      <div class="cards three">
        <article class="card">
          <h3>算法版本</h3>
          ${algorithms.items.map((item) => `
            ${renderMetric("versionKey", item.versionKey)}
            ${renderMetric("fixtureOnly", item.fixtureOnly)}
            ${renderMetric("status", item.status)}
          `).join("")}
        </article>
        <article class="card">
          <h3>回测批次</h3>
          ${backtests.items.map((item) => `
            ${renderMetric("id", item.id)}
            ${renderMetric("checkpoints", item.checkpoints.join(", "))}
            ${renderMetric("syntheticOnly", item.syntheticOnly)}
          `).join("")}
        </article>
        <article class="card">
          <h3>回测指标</h3>
          ${detail ? Object.entries(detail.metrics).map(([key, value]) => renderMetric(key, value)).join("") : "<p>暂无回测详情。</p>"}
        </article>
      </div>
      ${detail ? `
        <div class="table-wrap">
    ...
    setM3ErrorPageState("m3-backtests", error);
    document.querySelector("#m3BacktestsContent").innerHTML = renderM3Error(error);
  }
}
...
function showPage(page) {
  ...
  if (parsed.page === "m3-detail" && parsed.id) {
    state.m3SelectedTopicId = parsed.id;
  }
...
function renderCurrentPage() {
  ...
  if (state.activePage === "m3-overview") {
    return renderM3Overview();
  }
  if (state.activePage === "m3-topics") {
    return renderM3Topics();
  }
  if (state.activePage === "m3-detail") {
    return renderM3Detail();
  }
  if (state.activePage === "m3-gaps") {
    return renderM3Gaps();
  }
  if (state.activePage === "m3-backtests") {
    return renderM3Backtests();
  }
  return renderSystem();
}
