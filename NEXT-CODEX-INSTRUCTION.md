...
`复核已完成的 formal-cash target correction、commitment as-of 审计和 old→new target bridge；停在 C2-R.1 重新训练前，等待用户明确授权，不得自动训练/调参、进入 C2/C3/M3、打开 final holdout 或 release`
...
- C1 结构验证、同 case-key、无泄漏、future perturbation、raw/served/abstention、C1-only interval 和脱敏契约全部通过；legacy-target C2-R 随后已执行但结论为 `FAIL`，旧 development 写入口已 fail-closed，只保留历史验证入口。C2-R.1 尚未训练或调参，C2/C3 未开始；final holdout、embargo 和 60 月标签仍 sealed，状态保持 `not_for_formal_decision`。
...
`停在已验证的 formal-cash target correction checkpoint，先报告 commitment evidence 缺口、三套 actual 和 old→new bridge，等待用户明确授权后才能重新运行 C2-R.1；不得自行训练/调参、进入 C2/C3、打开 final holdout、release 或进入 M3`
...
5. C1 与 legacy-target C2-R 均已完成并 FAIL；不得重复训练、改 gate、自动训练或调参 C2-R.1、进入 C2/C3、打开 final holdout 或 release。下一算法动作必须由用户在复核正式现金基础设施和历史 supersession 证据后另行明确授权。M2 输出继续禁止自动运营建议或资源投入动作字段。
