...
The phase statements above and below preserve the original M2-A boundary for historical traceability. For final-algorithm calibration, the frozen decisions in `docs/analysis/m2-real-data/M2-calibration-scoring-eligibility-correction-decision-v1.md`, the committed machine-readable `src/domain/oldProductEvaluation/calibrationSpec.v1.1.amendment.json` (`calibration-spec-v1.1-amendment`), and its digest-bound base `src/domain/oldProductEvaluation/calibrationSpec.v1.json` (`calibration-spec-v1`) take precedence over older forecast-output, baseline, and gate descriptions. The amendment corrects scoring and eligibility semantics only; undeclared base model, origin, seed, bootstrap, output, release, and M3 boundaries remain frozen. The current authorized work is isolated local calibration only. It does not approve a candidate for formal decision or release and does not authorize M3.
...
Before corrected baseline replay or candidate training, the committed machine-readable `src/domain/oldProductEvaluation/calibrationSpec.v1.1.amendment.json` must bind the exact path, version, canonical digest, and introducing commit of `calibration-spec-v1`. The pair freezes every model and parameter, B0-B3 definition, candidate definition, statistical-scoreability rule, business-serving rule, abstention rule, stratum definition, seed, horizon, comparator, bootstrap rule, and acceptance gate. The v1.1 amendment is limited to the scoring/eligibility correction authorized after the first baseline diagnostic and before any candidate or final holdout was opened; it may not change model formulas, origins, seeds, bootstrap design, or relax a gate. Any result-dependent edit after this amendment is frozen requires a new amendment version and must keep C1 stopped. Any change after opening the final holdout requires a new full spec and a new untouched holdout.
...
The B0a-to-B0b attribution report is audit-only and must use the frozen seven-stage bridge: recorded B0a; legacy model on the new fixed case-key intersection; as-of quantiles/priors; as-of rating/lifecycle/features; new eligibility; new abstention scoring; and complete B0b. Stages 2 through 7 use exactly the same development intersection keys and change only the named layer. The recorded B0a has no identical case keys, so its difference to Stage 2 is a non-causal historical-to-intersection bridge gap. A missing legacy artifact must be reported as not reconstructable, never fabricated, and no attribution stage may participate in comparator or candidate selection.
...
For baseline comparator selection, first identify the provisional lowest all-scoreable raw WAPE. A baseline is statistically equivalent to that provisional best if their relative WAPE difference is strictly below 1% or their paired work×origin bootstrap 95% CI includes zero. The simplest member of that equivalence set is selected under the frozen order `B1 < B2 < B3 < B0b`. `B0a` is excluded; B0b and B3 remain reported. The comparator is locked once and cannot be reselected per gate.
...
The pre-C1 top-10%-value served-revenue coverage gate remains at least 90%, computed over all-scoreable top10 cases with `max(actual,0)` revenue. It is model-independent and must be evaluated after eligibility is frozen and before C1. If it is below 90%, candidate training stops and the exact frozen hard-blocking reasons must be reported; the threshold must not be lowered and works must not be selected to hit the target.
...
Candidate training order remains `C1`, `C2-R`, `C2`, then `C3`, but C1 stays stopped until the v1.1 amendment is committed, B0a-to-B0b attribution is reported, corrected B0b-B3 replay passes case/state parity and future-perturbation checks, the 90% pre-C1 coverage gate passes, and the user explicitly authorizes continuation. Selection chooses the simplest candidate that passes every frozen gate. Every candidate, including a passing candidate, remains `not_for_formal_decision` until Chinese business sampling and explicit user approval.
