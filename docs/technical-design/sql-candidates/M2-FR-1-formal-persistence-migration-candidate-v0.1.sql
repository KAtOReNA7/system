-- MIGRATION CANDIDATE ONLY - NOT A FORMAL MIGRATION
-- M2-FR-1 formal persistence candidate.
-- Do not place this file in db/migrations/ until it has passed the formal migration review.
-- Do not execute this file against production, staging, shared, or formal databases.
-- PostgreSQL 16 style; intended to align with existing m1 schema conventions.
...
COMMENT ON TABLE m2_evaluation_results IS
  'M2 formal readiness candidate table for old-product evaluation result snapshots. Candidate only; not yet a formal migration.';

COMMENT ON TABLE m2_evaluation_input_snapshots IS
  'M2 candidate input snapshot table storing aggregate inputs and version references, not raw bill rows.';
