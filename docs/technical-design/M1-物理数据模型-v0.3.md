...
> 状态：**HISTORICAL ARCHIVE — SUPERSEDED BY `M1-物理数据模型-v0.4.md`**  
> 本文件仅保留历史设计脉络，不再作为迁移实现、DDL候选或数据库审查依据；所有实现以 v0.4 为唯一基线。

| 项目 | 内容 |
|---|---|
| 状态 | ID RULES SYNCHRONIZED；允许进入非正式迁移 SQL 原型设计，不得生成正式迁移文件 |
| 日期 | 2026-06-21 |
| 数据库 | PostgreSQL 16+ |
| 金额类型 | `NUMERIC(32,18)` |
| ADR | `docs/technical-design/ADR-M1-数据库选型.md` |
| 基线 | `M1-物理数据模型-v0.2.md`、`M1-技术设计草案-v0.3.md`、现有 PRD、真实数据分析、运营确认包 v2.3 候选逻辑审计 |
| 不包含 | 正式迁移文件、应用代码、接口、页面、正式数据库、正式数据导入、运营确认结果自动应用 |

## 1. v0.3 修订结论

v0.3 保留 PostgreSQL 16+ 与 `NUMERIC(32,18)`，并在 v0.2 基础上完成以下同步修订：
...
- 基础信息版本采用“基础信息作用域全量快照”：每个 active 版本可独立读取已正式进入基础信息版本作用域的全部作品基础信息，但不要求每个账单新增标准作品在同一账单批次激活时同步复制基础信息；未进入基础信息版本的作品由缺口视图识别；
- 增加问题运行上下文、任务重试链和多消费者影响消费记录；
- 正式收入事实的七个业务字段及来源工作表全部 `NOT NULL`；临时记录仍允许空值并生成问题；
- 作品 ID 与业务形态规则冻结：完整原始作品 ID 按账单文本原样识别，标准作品 ID 使用数字主体，业务形态只由完整原始作品 ID 解析，授权分类不得反向覆盖业务形态；
- 增加 `system_state` 控制对象；仅 `schema_initialized`、`master_data_initializing` 允许 0 个 active 版本，`ready_for_bill_activation`、`operational` 阶段映射、基础信息、分类 release 和标签 release 必须各恰好 1 个；
- 历史分册映射在同一映射版本内按完整历史原始 ID 唯一；常规原始 ID 映射与历史分册映射通过 deferred constraint trigger 保证同版本跨表互斥；
- Flyway SQL-only 冻结为迁移工具，正式迁移继续采用 forward-only 原则；
- 物理表口径明确为 **47 张 M1 业务表 + 1 张 `system_state` 控制表 = 48 张物理表**。
...
- 当前映射仅由 `mapping_version.status='active'` 决定；`schema_initialized`、`master_data_initializing` 允许 0 个 active 映射版本，`ready_for_bill_activation`、`operational` 必须恰好 1 个。
- 当前基础信息仅由 `basic_info_version.status='active'` 决定；`schema_initialized`、`master_data_initializing` 允许 0 个 active 基础信息版本，`ready_for_bill_activation`、`operational` 必须恰好 1 个。
- 当前分类与标签版本分别由 `classification_release.status='active'`、`tag_release.status='active'` 决定。
- 映射版本为全部映射与投影完整快照；基础信息版本为“基础信息作用域全量快照”。快照行不再承担行级生效区间语义。
- 派生结果必须直接携带来源版本主键；不存在无来源版本的“当前缓存”。

### 2.4 作品 ID 与业务形态解析原则

- 完整原始作品 ID 必须按账单文本原样识别：`12345` 与 `Y12345` 是两个不同的原始作品 ID。
- 标准作品 ID 使用数字主体：`12345` 和 `Y12345` 均可映射到标准作品 ID `12345`。
- 业务形态只根据完整原始作品 ID 判断：`^[0-9]+$` 为 `audio_copyright`（有声版权），`^Y[0-9]+$` 为 `audio_product`（有声成品）。
- 授权分类只作为账单原始事实和问题检查字段，不得反向覆盖业务形态。
- 同一标准作品同时存在纯数字原始 ID 和 Y 前缀原始 ID 是正常双业务形态，不得仅因此生成导入阻断。
- DDL 侧通过受控函数/约束触发器集中调用等价于 `derive_standard_work_id(raw_work_id)` 与 `derive_business_form(raw_work_id)` 的单一权威解析逻辑；不得在多个触发器、视图或脚本中散落不同正则。
- 非法 ID 格式返回明确无效结果并生成数据问题；不得猜测修正、去前缀或用授权分类补判。

## 3. 物理表总表

| # | 表名 | 职责 | 属性 |
|---:|---|---|---|
| C | `system_state` | 系统生命周期与初始化门禁 | 控制权威 |
...
表数量口径：上表中编号 1-47 为 M1 业务表；`system_state` 为独立控制表。合计 48 张物理表。

## 4. 表定义

### 4.0 `system_state`

- 职责/属性：记录系统生命周期状态，作为初始化、首版激活和运行期“恰好一个 active 版本”约束的控制权威。
- 字段：`id smallint not null default 1`；`lifecycle_status text not null default 'schema_initialized'`；`status_changed_at timestamptz not null default now()`；`status_changed_by text null`；`status_note text null`。
- 主键/外键：PK `id`；无 FK。
- 唯一约束：主键；检查 `id=1`，保证全库只有一行控制状态。
- 检查约束：`lifecycle_status in ('schema_initialized','master_data_initializing','ready_for_bill_activation','operational')`。
- 索引：主键足够。
- 状态语义：
  - `schema_initialized`：schema 已创建，允许 0 个 active 映射版本、基础信息版本、分类 release 和标签 release。
  - `master_data_initializing`：正在装载分类、标签、基础身份和首个候选版本，仍允许 0 个 active 版本。
  - `ready_for_bill_activation`：映射、基础信息、分类 release 和标签 release 已由受控初始化函数在同一事务中各激活一个；不允许 0 个 active 版本。
  - `operational`：正式运行；映射、基础信息、分类 release 和标签 release 必须各恰好 1 个 active 版本。
- 约束实现：部分唯一索引保证“最多一个 active”；deferred constraint trigger 在 `ready_for_bill_activation` 和 `operational` 状态下校验四类对象各恰好一个 active，仅在 `schema_initialized` 和 `master_data_initializing` 允许 0 个。`master_data_initializing -> ready_for_bill_activation` 的受控函数必须原子创建或校验四类 bootstrap 版本、激活并切换状态；不得通过禁用约束或人工临时改库绕过初始化。首个账单批次激活时，bootstrap mapping 退休、包含首批事实投影的真实 mapping 激活、批次激活并切换到 `operational`，均在同一短事务完成；其余三个 bootstrap 版本可继续 active。
- 保留：永久；只允许受控初始化/状态切换函数修改。
- 对应：`AT-M1-003/004/007/027/040/041/052`。
...
- 唯一约束：主键。
- 检查约束：`business_form in ('audio_copyright','audio_product')`；业务形态必须来自完整原始作品 ID 的单一权威解析函数，不得由授权分类推断。
...
- 检查约束：原始 ID 非空白；业务形态值域；`mapping_source in ('id_rule','ops_confirmed')`；`standard_work_id` 和 `business_form` 必须与完整 `raw_work_id` 的权威解析结果一致，运营确认只能处理归属和异常，不得用授权分类覆盖业务形态。
- 跨表约束：与 `historical_volume_mapping` 通过 deferred constraint trigger 保证同一 `mapping_version_id` 内相同完整原始 ID 不会同时登记为常规映射和历史分册映射；触发器在提交前锁定对应 `mapping_version` 行并检查两表并集唯一性。
...
- 唯一约束：`UNIQUE(mapping_version_id, historical_raw_work_id)`；同一个映射版本中，完整历史原始作品 ID 不得因 `business_form` 不同而重复。
- 检查约束：历史原始 ID 非空白；业务形态值域；`business_form` 必须与完整 `historical_raw_work_id` 的权威解析结果一致；`target_standard_work_id` 来自运营确认的历史分册目标，不强制等于历史原始 ID 的数字主体；禁止同时在同一版本的 `raw_work_id_mapping` 中作为常规 ID。
- 跨表约束：使用 deferred constraint trigger 检查 `raw_work_id_mapping.raw_work_id` 与 `historical_volume_mapping.historical_raw_work_id` 在同一 `mapping_version_id` 内互斥；触发器对对应 `mapping_version` 行执行 `FOR UPDATE`，序列化同版本映射变更。
...
- 职责/属性：基础信息作用域内标准作品当前基础信息的全量快照版本；权威。
...
- 激活约束：deferred trigger/受控过程检查该版本对“已进入基础信息正式版本作用域”的标准作品各有一行，并且分类、标签分配仅引用本版本指定 release。账单批次中新出现、尚未完成基础信息补全或正式应用的 `standard_work` 不阻断账单批次激活，由缺口视图识别为待补全。
...
- 职责/属性：基础信息作用域全量版本内每部已正式纳入基础信息版本的标准作品一行基础信息；权威快照。
...
- M2 完整性：不保存 `is_complete_for_m2`。视图 `v_basic_info_m2_completeness` 以 `standard_work` 为左表，左连接 active 基础信息版本、分类和标签，按版本重算名称、作者、完整三级分类、版权日期和已冻结的必需标签；当前不存在基础信息记录的作品返回缺失，而不是从结果集中消失。必需标签配置仍 PENDING-DATA 时返回 `pending_configuration=true`，不得推断为完整。
- 保留：随版本永久。
...
3. 基于当前 active 基础信息版本创建新的“基础信息作用域全量” building 快照；未在本次补全文件中变化、但已经进入正式基础信息作用域的作品也复制到新版本。
4. 新账单批次可先独立创建 `standard_work` 身份；若该作品尚未进入基础信息版本，本次快照可以不包含该作品，后续由缺口视图和补全导出识别。
5. 引用明确的 `classification_release_id` 和 `tag_release_id`，构建作用域内全部作品行、分类分配和标签分配。
6. 校验作用域内每个标准作品在快照内恰有一行、引用版本一致、checksum 正确后改为 validated。
7. 短事务使用 `pg_advisory_xact_lock(hashtext('m1:basic-info-switch'))`，锁定旧/新版本和 apply batch；retired 旧版本、active 新版本、apply batch active，并写影响事件。

`schema_initialized`、`master_data_initializing` 允许没有 active 基础信息版本；进入 `ready_for_bill_activation` 前，受控初始化函数必须原子激活 bootstrap 基础信息版本。`ready_for_bill_activation` 与 `operational` 均要求全库恰好一个 active 基础信息版本。失败时旧 active 版本保持可见；回滚通过再次切换到从旧快照复制的新版本完成，不修改历史 active 快照。
...
- `v_basic_info_gap`：以 `standard_work` 为左表，识别尚未进入 active 基础信息版本、或缺少作者/三级分类/版权期限/必需标签的作品，用于补全导出与 M2 门禁。
...
1. 控制与无业务 FK 的基础表：`system_state`、`background_task`、`file_fingerprint_registry`、`cleaning_rule_version`、`channel`、`standard_work`、`author`、`classification_system`、`classification_release`、`tag_release`；
...
迁移工具冻结为 Flyway SQL-only。所有正式迁移采用 forward-only 原则，已执行迁移不得修改；修复通过后续前向迁移完成。本文不生成任何正式或非正式 SQL 文件。

### 11.2 数据装载顺序

1. 初始化 `system_state='schema_initialized'`，创建 schema 与迁移元数据；
2. 切换到 `master_data_initializing`，初始化规则版本、分类体系身份和业务形态值域；
3. 装载已确认渠道、标准作品身份，不要求 3,099 部基础信息已补齐；
4. 在 `master_data_initializing` 内构建或校验 bootstrap 分类 release、标签 release、基础信息版本和映射版本，此时允许 0 个 active 版本；
5. 处理正式导入阻断组，完成关键字段异常修正；
6. 账单上传到 staging，生成新问题实例；
7. 问题全部解除后构建批次事实；
8. 构建首个完整映射快照、投影和首次正数实销指标；
9. 受控初始化函数在同一事务中激活四类 bootstrap 版本并切换到 `ready_for_bill_activation`；该状态不允许 0 个 active 版本；
10. 严格对账和恢复点完成后，原子退休 bootstrap mapping、激活包含首批事实投影的真实 mapping、激活首批批次并切换到 `operational`；其余三个 bootstrap 版本可继续 active；
11. 装载月份完整性确认；
12. 根据运营确认结果逐批构建基础信息作用域全量版本；M2 前再完成对应作品必需字段。
...
- 检查点 D：基础信息作用域全量快照覆盖和引用 release 一致性；失败保留旧 active 版本。
...
- 本文 47 张 M1 业务表 + 1 张 `system_state` 控制表、主外键、唯一约束、状态检查、索引和触发器边界；
- `NUMERIC(32,18)`、自然月 `date`、不可变收入事实；
- 完整映射快照、基础信息作用域全量快照、分类/标签 release；
...
v0.3 已完成作品 ID/业务形态规则、初始化 active 版本语义、基础信息作用域和迁移工具结论同步。当前允许进入“非正式迁移 SQL 原型”设计与隔离验证准备；仍不得生成正式迁移文件、连接正式数据库、导入正式数据或自动应用运营确认结果。

正式迁移工具冻结为 Flyway SQL-only；正式迁移采用 forward-only 原则。非正式原型若生成，必须位于明确标记的实验目录或临时隔离环境，不得放入正式 `db/migrations/` 并不得被视为正式迁移历史。