...
| 组合预测（PORT） | 模型（model） | 分层收入组合模型 v0.1（Layered Revenue Composition Model v0.1，`M2-PORT-LRC01`） | `layered revenue composition`、`M2 layered revenue composition v0.1` | 已执行失败候选（`failed_development_candidate`） | 前序 `M2-WORK-CRMR01`、`M2-PORT-ETS01`；无后续 |
