...
| 渠道预测（CHAN） | 登记实体（model_revision） | 出版行业适配的渠道月度发生—条件金额核心（Publishing-Scale Channel Monthly Occurrence × Conditional Amount Core，`M2-CHAN-PSC01`） | `M2-current-publishing-scale-channel-v0.1`、`publishing-scale channel core`、`publishing scale occurrence amount core` | 登记角色（`development_candidate_implemented_not_executed`） | 前序 `M2-CHAN-GEN02`；无后续 |
