...
| 核心收入人工规则基线开发评价（Core-Revenue Manual Rule Baseline Development Evaluation，`M2-EXP-CORE-REVENUE-MANUAL-01`） | 核心收入人工规则原始候选 / MANUAL_RULE（`M2-EXP-CORE-REVENUE-MANUAL-01/MANUAL_RULE`；`PUBLIC_CANONICAL_IMPLEMENTATION_COMPLETE_PRIVATE_EVALUATION_NOT_EXECUTED`）；冻结人工锚定可学习全局比较基线 / RESEARCH_BASELINE（`M2-EXP-CORE-REVENUE-MANUAL-01/RESEARCH_BASELINE`；`FROZEN_COMPARATOR_NOT_REEXECUTED`）；冻结作品发生—金额运行回退比较基线 / OPERATIONAL_FALLBACK（`M2-EXP-CORE-REVENUE-MANUAL-01/OPERATIONAL_FALLBACK`；`FROZEN_COMPARATOR_NOT_REEXECUTED`） |
