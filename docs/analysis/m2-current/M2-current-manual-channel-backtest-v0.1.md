...
- 人工规则 WAPE 为 `0.704447`，高于 `0.30` 门槛；
- signed bias 为 `-0.290983`，绝对值高于 `0.10` 门槛，
  且不同 origin 的低估程度不稳定；
- 379 个可评分安全窗口 case 中，没有可用于 36 个月回测的耽美 case；
- 341 个 case 的前两大渠道收入占比
...
|---|---:|---:|---:|---|
| 最近一年收入×3 | 379 | 1.091150 | 0.389617 | FAIL |
| 人工规则原式 | 379 | 0.704447 | -0.290983 | FAIL |
| 主力年度水平再乘80% | 379 | 0.713331 | -0.413682 | FAIL；偏差恶化 |
| 生命周期固定50% | 379 | 0.699194 | -0.305467 | FAIL |

人工原式相对简单外推的 WAPE 改善
`35.44%`，说明业务规则确有信息；但绝对误差仍然
过大，不能直接替换统计模型。
...
| 2019-12 | 100 | 0.737122 | -0.106870 |
| 2020-06 | 214 | 0.728307 | -0.484192 |
