...
这里的 trailing-12 是截至 forecast origin 的最近最多 12 个可见月份的分成现金
之和；已经满足三个月成熟门槛但历史不足 12 个月时，使用当时已有月份，窗口内没有
入账的月份按零现金处理。不得读取 origin 之后月份，不得因不足 12 个月排除同案例，
也不得改用 CHAM01 为完整特征窗口保留的 `null`。这是对既有“起点可见
trailing-12 分成现金”合同的接线语义明确化，不改变人口、actual、实验臂、参数网格
或评价门禁。
