...
| 核心老品—已有渠道范围纠偏、冻结重评分与尾部干扰验证（M2 Core Legacy Work–Observed Channel Scope Correction, Frozen Rescore and Tail Interference Test，`M2-EXP-CORE-LEGACY-POPULATION-01`） | 范围治理完成、重评分与消融已预注册（`SCOPE_GOVERNANCE_COMPLETE_RESCORE_AND_ABLATION_PREREGISTERED`） | 当前范围合同和训练语义审计已完成；冻结重评分与一次训练人口消融仍等待阶段门禁，此处不登记新的模型优劣结论。 |
