...
2026-07-29 总账增补：核心收入人工规则基线 v0.1
（`M2-WORK-CRMR01`）的首个有效真实滚动评价已登记为长期复合失控失败。此增补只
...
| 核心老品—已有渠道范围纠偏、冻结重评分与尾部干扰验证（M2 Core Legacy Work–Observed Channel Scope Correction, Frozen Rescore and Tail Interference Test，`M2-EXP-CORE-LEGACY-POPULATION-01`） | 范围治理与冻结重评分完成、消融已预注册（`SCOPE_GOVERNANCE_AND_FROZEN_RESCORE_COMPLETE_ABLATION_PREREGISTERED`） | 当前范围、训练语义和冻结模型正确人口重评分已完成；不同 horizon 的同案例最低 WAPE 模型不同，不能拼成跨 horizon 冠军。一次训练人口消融仍等待阶段门禁。 |
...
| `CG-G1-BLOCKED-NO-CANDIDATE-OUTCOME` | 仅独立展示（`STANDALONE_ONLY`） | 渠道时间生成模型 v0.2——独立渠道月度发生—条件金额核心（Channel Generative v0.2 — Independent Monthly Occurrence × Conditional Amount Core，`M2-CHAN-GEN02`）没有候选分数（`M2_CHANNEL_GENERATIVE_G1_CORE_BLOCKED`） |
