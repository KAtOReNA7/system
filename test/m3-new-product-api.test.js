...
test("M3 overview API returns fixture stage summary and distributions", async () => {
  ...
  assert.equal(response.body.summary.totalTopics, 5);
  ...
  assert.equal(response.body.distribution.readiness.ready, 3);
...
test("M3 topic list supports pagination filters and sorting", async () => {
  ...
  assert.equal(response.body.items[0].topicId, "SYN-TOPIC-0001");
...
test("M3 topic detail exposes material comparator forecast rating link and backtest sections", async () => {
  ...
  assert.equal(response.body.comparators.filter((item) => item.selectedAsFinal).length <= 3, true);
