...
import { inspectOptionalPrivateIdentity, requireRegisteredArtifact } from "./helpers/m2V2RequiredArtifacts.js";
...
test("C2 development preserves the exact population, prediction lock, and seals", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
...
test("C2 reports all fixed quality gates and independent decisions", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
  const report = JSON.parse(fs.readFileSync(validationPath, "utf8"));
...
test("C2 segment, residual, and high-value reports preserve their frozen contracts", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
...
test("C2 remains comparable with B4, B0b, B1, and B3 on identical keys", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
  const report = JSON.parse(fs.readFileSync(validationPath, "utf8"));
...
test("C2 public artifacts are Chinese, aggregate-only, and omit PI endpoints", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
  const names = [
...
if (process.env.M2_V2_TEST_PROFILE === "optional-private") {
  test("ignored C2 evidence remains untracked and is verified when complete", (context) => {
    const optional = inspectOptionalPrivateIdentity(root, "OPT-C2");
...
test("C2 decisions do not authorize C3, release, or M3", () => {
  requireRegisteredArtifact(root, "C2_DEVELOPMENT_VALIDATION_JSON", { expectedPath: validationPath });
  const report = JSON.parse(fs.readFileSync(validationPath, "utf8"));
